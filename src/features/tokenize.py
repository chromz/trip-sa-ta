...
def tokenize_classes(document_classes: Dict[str, List[str]], load_bigrams: bool = True) -> Dict[str, List[List[str]]]:
    ...
    if load_bigrams:
        with open('models/bigrams.joblib', 'rb') as input_file:
            bigram_mod = joblib.load(input_file)
    else:
        words = []
        for word_class, words in word_classes.items():
            words.extend(words)
        bigram_mod = bigrams_model(words)
        with open('models/bigrams.joblib', 'wb') as output_file:
            joblib.dump(bigram_mod, output_file)
